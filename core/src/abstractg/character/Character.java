 ...
 **/
 ...
 public abstract class Character{
    ...
    public void setCellCollision(Fixture cell){

        this.cell = cell;

    }
    public  boolean isCellOfCollision(){
        return cell != null;
    ...
    public  boolean isForceCrouched(){
        ...
        Body c = cell.getBody();
        Vector2 v = (Vector2) c.getUserData();

        if ((getX() *AimosVars.PTM  + to2DBoxSize(getWidth())*1.25)>= (c.getPosition().x * AimosVars.PTM) &&
                (getX() *AimosVars.PTM  - to2DBoxSize(getWidth())*1.25) <= (c.getPosition().x * AimosVars.PTM) + (v.x * 32)){
            return  true;
        }else{
            cell = null;
        }
