...
import abstractg.control.Hud;
import abstractg.core.Launcher;

import abstractg.character.Player;
import abstractg.handlers.AimosContactListener;
...
public class Play extends GameState{
    ...
    private TiledMapTileLayer fxLayer,floorLayer,moreLayer,xFxLayer;
    ...
    protected Play(GameStateManager gsm) {
        ...
        bgMusic = Launcher.res.getMusic("city_l2");
    ...
    public void render() {
        ...
        floor.setPosition(player.getX() * AimosVars.PTM + Launcher.WIDTH / 4, Launcher.HEIGHT / 2);
        fx.update();
        xFx.update();
        floor.update();
        ...
        draw();
        if(debug) {
            b2dCam.setPosition(player.getX() + Launcher.WIDTH / 4 / AimosVars.PTM, Launcher.HEIGHT / 2 / AimosVars.PTM);
            b2dCam.update();
    ...
    private void createBlocks(TiledMapTileLayer layer, short bits) {
        ...
        for (int i = 0; i < coords.size ; i++) {
            BodyDef bdef = new BodyDef();
            bdef.type = BodyDef.BodyType.StaticBody;
            bdef.position.set(coords.get(i).x * ts / AimosVars.PTM, coords.get(i).y * ts / AimosVars.PTM);
